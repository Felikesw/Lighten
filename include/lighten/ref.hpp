...
class ref_ptr_base {
  ...
  void replace_ptr(uintptr_t ptr) const {
    uintptr_t p = lock_ptr();
    
    if(!atomic_compare_exchange_ptr(p, ptr)){
      reinterpret_cast<referenceable*>(ptr)->put_ref();
  ...
  void set_ptr(uintptr_t ptr) const {
    if(REF_PTR_IS_LOCKED(ptr)){
      throw ref_ptr_lock_error(); 
    }
    if(ptr){
      reinterpret_cast<referenceable*>(ptr)->get_ref();
    }

    replace_ptr(ptr);
  }

  uintptr_t move_ptr() const {
    uintptr_t p = lock_ptr();
    *m_ptr_p = 0;
    return REF_PTR_CLR_LOCK(p);
  }


  uintptr_t get_ptr() const {
    return ((*m_ptr_p)&REF_PTR_MASK);
  }
